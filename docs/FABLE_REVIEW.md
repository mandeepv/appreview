...
- **Kill switch `id0000000000` → 404:** replaced with real App Store ID `APPLEAPPID` in `ForceUpdateModal.tsx`.
...
Verified — real project refs (`devprojectref000000x` for dev, `prodprojectref00000x` for prod) are in the code. Placeholder strings only ever existed in the `appreview` sanitized copy. Env detection works correctly.
