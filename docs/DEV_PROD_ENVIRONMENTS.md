...
|---|---|---|---|
| **Prod** | `kinderwell` | `prodprojectref00000x` | us-west-2 |
| **Dev** | `kinderwell-dev` | `devprojectref000000x` | us-west-1 |
...
```bash
export DEV_DB_URL='postgresql://postgres.devprojectref000000x:PASSWORD@aws-1-us-west-1.pooler.supabase.com:5432/postgres'
...
```bash
export PROD_DB_URL='postgresql://postgres.prodprojectref00000x:PASSWORD@aws-0-us-west-2.pooler.supabase.com:5432/postgres'
...
1. **Start on dev.** `.env` already points at dev. Confirm with `cat .env | grep SUPABASE_URL` → should show `devprojectref000000x`.
...
cat .env | grep SUPABASE_URL   # should show devprojectref000000x
