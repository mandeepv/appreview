...
| PostHog | session replay — PII masking posture | ⚠️ **PARTLY UNBUILT — this row overstated reality until 2026-09-15.** RN replay is SCREENSHOT-based, so masking IS the PII control (INVARIANTS: no name/child data to PostHog). What EXISTS: the config flags in `src/config/posthog.ts` — `maskAllTextInputs:true`, `maskAllSandboxedViews:true` (Superwall), `maskAllImages:false`. What DOES NOT exist: the explicit `<PostHogMaskView>` wrappers this row claimed were "built 2026-07-21" on the name-bearing titles and child-count/age values. `grep -rn PostHogMaskView src/` returns NOTHING, and it is absent from `main` and from the `v1.2.0-build-11` / `appstore-live-v1.2.0` tags too — so it was never shipped and the 2026-09 redesign did not remove it. Harmless while replay is OFF (the row below), but **the wrappers must be built before the dashboard toggle is flipped**, or replays capture the parent's name and children's ages as screenshots. Rebuilt You screen renders name + "Mom to two children"; onboarding renders name/age/children — all currently unmasked. | **2026-09-15 (corrected)** | `grep -rn PostHogMaskView src/` — expect hits before enabling replay |
