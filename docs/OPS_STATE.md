...
| PostHog | **session replay — NOT RECORDING; needs 3 owner actions** | **TWO independent switches. (a) In code: ON IN DEV, OFF IN PROD** — `ENABLE_SESSION_REPLAY = environment === 'dev'` in `src/config/posthog.ts`, bound to the Supabase project ref baked in at build time. The PROD binary cannot record regardless of the dashboard, because the shared dev+prod PostHog project means a plain `true` would capture live App Store users who have consented to none of this. (b) In the dashboard: PostHog → Settings → Project → Session Replay (+ enable for mobile/React Native). Nothing records until BOTH are on. ⚠️ Before flipping either, three OWNER-ONLY items are required — RN replay is screenshot-based (PostHog docs: "always record in screenshot mode"), so it is a screen recording under App Review **2.5.14** ("explicit user consent and a clear visual and/or audible indication when recording... user activity") and **5.1.1(ii)**: (1) a consent flow + indicator, (2) privacy-policy update disclosing the recording and naming PostHog as recipient (5.1.1(i)/5.1.2; `legal/` is owner-only), (3) App Privacy label — Name is declared AppFunctionality/Personalization, not Analytics. Shipping the code is safe; enabling without (1)-(3) risks rejection of a LIVE app. ⚠️ Single PostHog project (dev+prod) — enabling records BOTH; filter on the `environment` super-property. Plugin `@posthog/react-native-plugin` 2.9.0 installed + registered in app.json → **needs a native rebuild**. | **2026-09-15 (dev ON for testing, prod OFF)** | `grep -n ENABLE_SESSION_REPLAY src/config/posthog.ts` + PostHog dashboard |
| PostHog | session replay — PII masking posture | **Built 2026-09-15** (the row previously claimed this was done 2026-07-21; it was not — `PostHogMaskView` wrappers were never in `main` or the shipped `v1.2.0-build-11`). What exists NOW: `enableSessionReplay` + `sessionReplayConfig` in `src/config/posthog.ts` with PostHog's restrictive defaults kept — `maskAllTextInputs:true` (masks the typed NAME on NameAgeScreen), `maskAllImages:true`, with `captureLog:true` / `captureNetworkTelemetry:true` — a replay without the console trail and request timings is a silent film, and masking (not blinding the replay) is the PII control. Standing rule that keeps it safe: never console.log a token or an email. Tapped ANSWERS stay visible (buttons, not inputs) — that is intentional and is the point of replay. | 2026-09-15 | `grep -n sessionReplayConfig src/config/posthog.ts` |
