...
| PostHog | onboarding A/B experiment dashboard [SPEC-15 §4.3] | **pending (dashboard build only — PLUMBING VERIFIED 2026-07-22)** — funnel `welcome_cta_tapped`→`onboarding_step_completed`→`auth_succeeded`→`paywall_presented`→`subscription_purchased` broken down by `onboarding_variant`, env=production; + `onboarding_variant_assigned` by `source` trend. ✅ Code side is fully wired: `subscription_purchased` fires to PostHog on Superwall purchase (`LoadingScreen.tsx:292`), and `onboarding_variant` is a registered super-property auto-attached to EVERY event (`experiments.ts:226`, re-registered on app start, deliberately never unregistered) — so every purchase event is variant-tagged. NO DB column needed to measure conversion; conversion truth is the PostHog funnel, not a `user_profiles` flag. Remaining work is building the funnel in the PostHog UI (owner/dashboard action), filtered to env=production so dev testing doesn't pollute it | 2026-07-22 (plumbing) | PostHog → Dashboards |
...
| Variant B rating | native review prompt WIRED + timing fix | **done 2026-07-21, timing fixed 2026-07-24** — `expo-store-review` installed; `VBRating`'s "Rate Kinderwell" calls `StoreReview.requestReview()` (guarded by `isAvailableAsync()`). **Timing fix:** when the prompt is available we hold the rating screen ~1.2s (`RATING_SHEET_DWELL_MS`) so the OS sheet animates in over the RATING screen, not the next one; when unavailable we navigate immediately; either way never stuck. Apple gives NO callback on whether the sheet showed/was tapped/was suppressed (rate-limited ~3×/yr) — that black box is by design, so a fixed dwell is the correct pattern; we deliberately do NOT try to detect the rating. Native dep → needs a native rebuild. ⚠️ still-placeholder: "100,000+ parents" + Sarah/James testimonials (see row above) | 2026-07-24 | `grep expo-store-review package.json` |
| Variant B reminders | OS permission WIRED, sequence UNDEFINED | **partial 2026-07-21** — `VBReminders` "Enable reminders" now requests the OS notification permission on tap (`expo-notifications`) and stores the grant. **The reminder SEQUENCE is NOT defined and nothing is scheduled** — frequency, time-of-day, copy, and tap target are all undecided (owner: "decide later"). A user who taps Enable grants permission but receives NOTHING until the sequence is defined + scheduling is built. Do NOT imply reminders work until then. Tracked in BACKLOG #13 + parked SPEC-11 | 2026-07-21 | BACKLOG #13 · `grep -rn scheduleNotification src/` |
| Variant B design | visual redesign (warm "Headspace/Noom" system) | **in progress, code-side done 2026-07-25** — variant B onboarding was rebuilt from the flat white "toy" look to a warm cream-canvas system. New variant-B-ONLY components (variant A stays pixel-stable, untouched): `StoryScreen` (hero narrative beats: illustration/line-icon chip + big left headline + `HighlightText` accent numbers + two-beat entrance), `VBQuestionScreen` (warm question shell), `AnalyzingTheater` (richer personalized calculate beat that reflects the user's own answers back, distinct from the shared PlanTheater which stays frozen), plus `warm` appearance on OptionCard/OptionList and icon-circle SnapshotCard. All copy rewritten human (no em-dashes/AI tics). PostHog session-replay masking updated for the new name/child-age surfaces. ⚠️ ART: real illustrations are the `_illo.jpg` set (11 distinct); the bare-name `.png` twins (md5 7143bd42…) are placeholder dupes — do NOT wire those. See `docs/specs/variant-b-illustration-style.md` for the style guide + generation prompts for the still-missing slots | 2026-07-25 | `docs/specs/variant-b-illustration-style.md` · walk variant B on device |
