...
- **Name:** `kinderwell-dev`
- **Project ref:** `devprojectref000000x`
- **URL:** `https://devprojectref000000x.supabase.co`
- **Anon (publishable) key:** `sb_publishable_DEV_KEY_REDACTED`
...
- **Command used:** `supabase functions deploy delete-account --project-ref devprojectref000000x`
...
- Added Domain: `devprojectref000000x.supabase.co`
- Added Return URL: `https://devprojectref000000x.supabase.co/auth/v1/callback`
- **Existing prod entries preserved:** `prodprojectref00000x.supabase.co` and its callback URL
...
- **Callback URL (auto-shown):** `https://devprojectref000000x.supabase.co/auth/v1/callback`
...
- Added Authorized redirect URI: `https://devprojectref000000x.supabase.co/auth/v1/callback`
- **Existing prod URI preserved:** `https://prodprojectref00000x.supabase.co/auth/v1/callback`
...
- **`.env` now contains:**
  ...
  SUPABASE_ANON_KEY=sb_publishable_DEV_KEY_REDACTED
  SUPERWALL_API_KEY=pk_SUPERWALL_KEY_REDACTED   (unchanged — Superwall shared)
  SKIP_PAYWALL=false
...
- **What it does:** On startup in `__DEV__` builds only, logs one of:
  - `[Supabase] Env: DEV ✅ | Project: devprojectref000000x`
  - `[Supabase] Env: PROD ⚠️ | Project: prodprojectref00000x`
  - `[Supabase] Env: UNKNOWN | Project: <ref>`
- Silent in production/release builds (`__DEV__` false)

### 10. Supabase CLI relinked to dev
- **Before session:** linked to prod (`prodprojectref00000x`)
- **After session:** linked to dev (`devprojectref000000x`)
...
- [ ] Confirm local `.env` still points at dev: `cat .env | grep SUPABASE_URL` → should show `devprojectref000000x`
...
- [ ] Metro logs show `[Supabase] Env: DEV ✅ | Project: devprojectref000000x`
...
- `.env` — added `POSTHOG_PROJECT_TOKEN=phc_POSTHOG_TOKEN_REDACTED...` and `POSTHOG_HOST=https://us.i.posthog.com`
...
- Google Cloud Console → created new iOS OAuth client `Kinderwell iOS Dev` with bundle `com.kinderwell.app.dev`, team ID `OLDTEAMID00`
