...
> **✅ STATUS: COMPLETE & VERIFIED — executed 2026-08-21, archived 2026-08-24.** The transfer is done: app on the account holder's team `APPLETEAMID`, 3,774 Apple users migrated + verified at scale (0 dupes/orphans/lost), window-signup gap checked clean (0), Superwall/EAS/auth all cut over. This is now a FROZEN HISTORICAL RECORD — read for the story, do not re-execute. The recurring dates that remain live are in **`docs/REMINDERS.md`**.
>
> **📁 PATHS IN THIS DOC ARE PRE-ARCHIVE.** This runbook + the migration scripts were moved into `docs/archive/app-transfer-2026-08/` on 2026-08-24. So every `scripts/apple-transfer/…` reference below now lives at `docs/archive/app-transfer-2026-08/scripts/…`. And any "local only / not committed" note is outdated — the tooling WAS committed (`c98f2b6`, `47b153d`, `1c5d18a`); only the ignored data files (csv/bridge/sql) remain uncommitted, kept locally as a recovery copy until the ~Oct 20 TN3159 window closes. (`scripts/generate_apple_jwt.js` did NOT move — it's an ongoing tool.)
>
> _Original pre-execution status (kept for context): the plan was ready with the § 3.3.1 dev-verification gate as the last technical blocker; all open questions (Q-SW/Q-TN/Q-SIWA) resolved; SBP 15% effective 2026-08-15 so no commission gap._
