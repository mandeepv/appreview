...
- **2026-07-04** — Structural prod guards (Fable P1#11, partial + Fable-2 🟡 first item). `supabase.ts` HARD THROWS on: dev build → prod DB, store build → non-matching DB. `app.config.js` HARD THROWS on: prod bundle + missing required env vars, prod bundle + non-prod SUPABASE_URL. Defaults flipped from prod-shaped to dev-shaped so a fresh clone / missing `.env` yields dev, not prod. The `ALLOW_DEV_PROD_ACCESS` env-var escape hatch was removed — bypassing the guard now requires an obvious code change on a scratch branch. **DB password rotation still deferred by user.**
