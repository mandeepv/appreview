...
- [ ] Confirm `.env` points at dev — `cat .env | grep SUPABASE_URL` shows `devprojectref000000x`
...
- [ ] Sanity-check that expected tables exist on prod (adjust the table
  ...
  ```bash
  curl -s -w "\n%{http_code}\n" \
    "https://prodprojectref00000x.supabase.co/rest/v1/app_config?select=*&limit=1" \
...
- [ ] **Supabase prod project ref is identical in all three files.**
  ...
  echo "=== app.config.js ===" && grep 'includes.*supabase\|prodprojectref00000x' app.config.js | head -2
  ```
...
- [ ] **In-app feedback:** long-press the 7-tap title area 3 times to open a `mailto:support@example.com?subject=Kinderwell%20Beta%20Feedback` composer. (**Not built yet** — for v1.1.0 testers, rely on the two channels below.)
- [ ] **TestFlight feedback:** TestFlight's built-in "Send Beta Feedback" button. Check daily during beta.
- [ ] **Direct email** to support@example.com. Homies have this already.
...
> 5. **Please report:** any crash, any confusing screen, any behavior that "feels wrong." Even small things. TestFlight has a "Send Beta Feedback" button, or email support@example.com.
...
- [ ] Fill out **TestFlight Test Information** (required by Apple Beta App Review, will reject otherwise):
  ...
  - Feedback Email: support@example.com
...
- [ ] Confirm `eas.json` has a `submit.production` block that includes:
  ...
  "submit": {
    "production": {
      "ios": {
        "ascAppId": "APPLEAPPID",       // Kinderwell in ASC (App Information → Apple ID)
        "appleTeamId": "APPLETEAMID"     // The Account Holder (Individual) — app transferred 2026-08-21
      }
    }
  }
  ```
