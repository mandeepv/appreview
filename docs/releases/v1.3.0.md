...
- [ ] Trigger the CI workflow on **`release/1.3.0`** (the commit you're
  releasing — NOT `main`, which is frozen at the previous version and would
  prove nothing about this release). CI's push/PR triggers don't fire on
  `release/*` branches, so this manual dispatch is the ONLY thing that runs CI on
  this code:
  - GitHub → **Actions** tab → **CI** workflow → **Run workflow** → pick
    `release/1.3.0` → **Run workflow**. Or from a terminal:
    `gh workflow run CI --ref release/1.3.0`.
...
- [ ] **Apply via `scripts/db-push-prod.sh` — NOT a manual `supabase link` + `db push`.**
  The script takes a fresh backup first, then pushes; a manual `db push --linked`
  skips that backup on a database with no automated backups (CLAUDE.md hard rule:
  prod pushes go through the script, and it's what the blocker note above
  mandates). The script prompts before applying.
  - [ ] Dry-run / preview what will apply (the script shows this, or:
    `supabase db push --linked --dry-run` while it is momentarily linked to prod).
    Confirm it lists ONLY the migrations you expect. If it lists something
    unfamiliar → STOP.
  - [ ] Run `scripts/db-push-prod.sh` during a low-traffic window; confirm it took
    a backup and applied cleanly.
  - [ ] The script re-links back to dev when done. Verify with
    `supabase migration list --linked` (should show dev). If it left you on prod,
    re-link immediately: `supabase link --project-ref devprojectref000000x`.
...
The Phase 2 CI run gates the branch at that moment — but code often changes
...
commit lands. v1.3.0 builds from **`release/1.3.0`** (before it merges to
`main`), so re-run the gate here on that branch's current tip.

- [ ] `git fetch && git log <last-CI-commit>..release/1.3.0 --oneline -- src/ supabase/functions/ app.json package.json`
...
- [ ] Re-run CI on the current `release/1.3.0` tip and confirm the 4 gating jobs
  are green (this is also the FIRST time CI runs on this branch — its push/PR
  triggers don't fire on `release/*`):
  ```bash
  gh workflow run CI --ref release/1.3.0 && sleep 5 && gh run watch
  ```
