...
**Release scope:** onboarding A/B test framework + variant-B scaffold (SPEC-15). First release off the `develop` train (v1.3.0→v1.6.0). The experiment is server-controlled (PostHog `onboarding-flow` flag); this binary just carries both arms, with variant_b at 0% until the placeholder copy is replaced.

**Release-specific verification for v1.3.0** (in addition to the standard checklist below — the SPEC-15 §5 device-test checklist):

- [ ] **Force `variant_b` for a test device** (PostHog flag override / test account) → Welcome routes into VariantBQ1→Q3 → Auth → theater → paywall; complete signup; the `user_profiles` row has `onboarding_variant='variant_b'` + a populated `variant_b_answers`. _(Evidence: on-device flow + prod/dev row screenshot.)_
...
- [ ] **Kill app mid-variant-B (on Q2)** → relaunch resumes at Q2 (not Welcome, not variant A). _(Evidence: on-device resume.)_
