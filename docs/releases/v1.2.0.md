...
> **INSTANCE — SHIPPED 2026-07-19.** A copy of `../RELEASE_CHECKLIST.md`, ticked AS v1.2.0 shipped with a date + one line of evidence per box. All ship-time phases (through Phase 10 tagging + prod test-user cleanup) are closed. The ONLY thing still open is Phase 11 post-release monitoring — an intentionally-ongoing 7-day rollout window (through ~2026-07-26), not a ship step. Otherwise FROZEN: do not re-litigate ticked boxes. Device-test boxes left in their as-shipped state (owner-run; not re-verified in this pass).
...
- [x] **Redeploy the hardened `delete-account` edge function — DEV-FIRST, secret required.** ✅ DONE 2026-07-11 (see sub-step 3 + acceptance checks below — deployed to BOTH dev + prod, prod is ES256/JWKS so no `JWT_SECRET` needed). This release redeploys `delete-account` (hardened in SPEC-03/FIX-01; +76/−32 vs live). It fails CLOSED without `JWT_SECRET`, so the ORDER matters:
  1. **Check prod's signing system FIRST** (SPEC-FIX-06): `curl -s https://prodprojectref00000x.supabase.co/auth/v1/.well-known/jwks.json`. If it advertises an **ES256/RS256** key → prod is on asymmetric signing keys and the function verifies via JWKS (**no `JWT_SECRET` needed on prod**). If the key set is empty/legacy → prod is still HS256 and `JWT_SECRET` **is** required. The dual-path code handles both; this check just tells you whether step-2's secret is mandatory.
...
- [x] **Single `paywall_presented` check** — ✅ 2026-07-11: `subscription_gate` confirmed Gated + no-dismiss in dashboard; on-device the paywall presented exactly once (Phase-2 gate single-fire check passed). _(Evidence: dashboard #1/#4 confirmed + on-device single-fire.)_
...
- [~] **Upgrade-in-place progress check** — strict paid old→new path CONSCIOUSLY SKIPPED for v1.2.0 (hard-paywall constraint: App Store build = real money, TestFlight only serves 1.2.0, no free content to build a pre-upgrade state — see RELEASE_CHECKLIST Phase 8.3 constraint box). Covered instead by: (a) Phase-2 dev evidence that account-scoped progress survives sign-out→sign-in via cloud (same path a real upgrader hits, verified on device 2026-07-11); (b) the direct sandbox flow on the 1.2.0 TestFlight build below. _(Evidence: dev sign-out/in progress-survival + 1.2.0 sandbox flow.)_
- [ ] **Offline-first-launch note (SPEC-FIX-08, accepted risk)** — a paying v1.1.0 user whose FIRST v1.2.0 launch is OFFLINE has the legacy-unowned flag cleared and sits at the retry/escape-hatch screen until connectivity returns (one launch, payers-only — inherent cost of the user-binding fix). Owner-accepted 2026-07-11; recorded in OPS_STATE Accepted-risks. No action needed — noted so device testers don't flag it as a bug.
...
3. **Phase 7.5** — Verify the `subscription_gate` paywall has **NO dismiss control** (no X, no "Not now," no swipe-to-dismiss) — the hard-paywall model. Apple 3.1.2 allows this for content-only paid apps provided Restore Purchases is visible + the demo bypass works. Do NOT add a dismiss control. (SPEC-FIX-10 F2 — the old "must have a dismiss control" wording was the opposite of the model.)
...
### Re-run CI on the exact shipping commit (release gate)

- [x] ✅ Re-ran CI on `main` @ `83789fe` — 2026-07-11 — 4 gating jobs green
  (TypeScript, ESLint, Version drift, Jest; dependency audit advisory/red-ok).
  Needed because the delete-account fix (`5bad869`) + version bump (`39601da`)
  landed after the Phase-2 CI run. See RELEASE_CHECKLIST Phase 7 for the
  standing step this near-miss produced.

### Pre-build sanity checks (Fable re-review 2026-07-05)

- [x] ✅ All three passed 2026-07-11: `appVersionSource=local`; prod ref
  consistent across eas.json / app.config.js / src/lib/env.ts; SENTRY_AUTH_TOKEN
  present in EAS production env.
...
- [x] ✅ Built 2026-07-11 — build `624d434d`, version 1.2.0 / build 10, production/store, from commit `fb3e01c` (CI-green). `.ipa` produced.

### Upload to TestFlight (`eas submit`) — REQUIRED before Phase 8

`eas build` only makes the `.ipa` in EAS; it does NOT reach TestFlight. Must
`eas submit` to get it to App Store Connect. (`eas submit:list` was empty after
the build — nothing sent to Apple yet.)

- [ ] `eas submit --profile production --platform ios` (eas.json `submit.production`
  block present: ascAppId APPLEAPPID, appleTeamId OLDTEAMID00). Answer export-
  compliance prompt if shown (HTTPS-only = exempt).
- [ ] Confirm 1.2.0 (10) appears in ASC → TestFlight, processes, then "Ready to Test".

> ⚠️ "Do NOT submit yet" = the FINAL Phase-9 App Store release, NOT this
> TestFlight upload. You MUST eas submit here for Phase-8 testing.

---

## Phase 7.5: Superwall dashboard verification (submission blocker)

> **v1.2.0 status (2026-07-11): ✅ verified.** Dashboard config for
> `subscription_gate` is unchanged since v1.1.0 (which shipped this same
> placement). Targeted re-check done — **#1 Feature Gating = Gated** and
> **#4 no dismiss control** both confirmed in the dashboard (the two settings
> whose silent drift would be catastrophic/invisible). The remaining items
> (prices visible, present-once, Restore Purchases) were confirmed **on-device
> 2026-07-11** during Phase-2 device testing — stronger evidence than a
> checkbox. NOTE this release DID rewrite `LoadingScreen.tsx` (the code that
> *presents* the placement, +414/−68), so the paywall *behavior* was
> re-tested on device even though the dashboard config didn't change.
...
- [x] **Phased Release for Automatic Updates** section → toggle **ON** — ✅ DONE 2026-07-19 (owner-confirmed; 7-day phased release enabled). Apple auto-rolls out over 7 days — ~1% day 1, ~2% day 2, etc. If crashes / bad reviews spike in the first days, you pause the rollout and only ~10% of users saw the bad build.
...
- [x] **Do NOT click "Release" immediately.** ✅ Released deliberately, not on-approval. Apple approved; build was in "Pending Developer Release" before release.
- [x] Install the approved build via TestFlight one more time and run the full smoke test on real device. ✅ (owner-confirmed clean before release.)
- [x] If clean → click **"Release This Version"** in App Store Connect. ✅ DONE 2026-07-19 (owner-confirmed 1.2.0 released).
- [x] **Tag the release** (dual-tag scheme — one build tag per build, one live marker per platform): ✅ DONE 2026-07-19.
  - [x] Build-specific tag on the exact shipped commit: `v1.2.0-build-11` on `39badd3`. ✅ (already on remote.)
  - [x] Move the production marker tag `appstore-live-v1.2.0` → `39badd3`. ✅ Created and pushed; previous marker `appstore-live-v1.1.0` deleted locally + on remote (one marker per platform).
  - [x] Push tags to GitHub. ✅ Both pushed to origin 2026-07-19.
...
- [x] Delete test users from prod Supabase → Authentication → Users. ✅ DONE 2026-07-19 — 1 test account deleted.

---

## Phase 11: Post-release monitoring (phased rollout window — 24-72h then 7 days)

> **STATUS: ACTIVE — window opened 2026-07-19** (7-day phased release enabled). Boxes below are left UNCHECKED on purpose: they are ongoing monitoring for the rollout window (~through 2026-07-26), not one-time ship steps. No pause threshold tripped as of the release-day snapshot. This is the ONE phase that legitimately outlives the "freeze after shipping" rule — close it out when the rollout hits 100%.
