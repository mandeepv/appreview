...
- [ ] **Apply the v1.2.0 migrations to prod** — via `scripts/db-push-prod.sh` (one prod push covers both): `supabase/migrations/20260710000000_add_completed_sections.sql` (SPEC-13) AND `supabase/migrations/20260710010000_rls_update_with_check.sql` (SPEC-FIX-04 R4 — WITH CHECK on the UPDATE policies). Then `npm run gen:supabase-types` and commit if the generated types differ (they shouldn't — R4 is RLS-only). _(Evidence: `supabase migration list` output + row count.)_
