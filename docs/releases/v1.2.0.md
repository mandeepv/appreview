...
- [ ] **Redeploy the hardened `delete-account` edge function — DEV-FIRST, secret required.** This release redeploys `delete-account` (hardened in SPEC-03/FIX-01; +76/−32 vs live). It fails CLOSED without `JWT_SECRET`, so the ORDER matters:
  1. **Set the secret** (owner-only for prod): `supabase secrets set JWT_SECRET=<Dashboard → Settings → API → JWT secret>`. ⚠️ Name is exactly `JWT_SECRET`, NOT `SUPABASE_JWT_SECRET` (the `SUPABASE_` prefix is reserved and rejected → the function would 500 on every call).
  2. **Deploy to DEV first**, set the dev secret, and run the four acceptance curls below. The hardened function has never been deployed anywhere — dev-first is mandatory.
  3. **THEN** set the secret on PROD and deploy to prod (`scripts/db-push-prod.sh` / the Phase-5 prod deploy).
  - Four acceptance curls (run against the deployed function; copy from `supabase/EDGE_FUNCTION_DEPLOYMENT.md`):
    - Tampered token → **401** (signature check rejects it).
    - Expired token → **401** (exp claim rejected).
    - Valid token → **200** + the user's rows deleted (lesson_progress + profile + auth user gone).
    - Secret UNSET on that environment → **500** (fail-closed; proves the prerequisite is load-bearing).
  _(Evidence: the four curl status codes.)_
