...
- [ ] **Redeploy the hardened `delete-account` edge function — DEV-FIRST, secret required.** This release redeploys `delete-account` (hardened in SPEC-03/FIX-01; +76/−32 vs live). It fails CLOSED without `JWT_SECRET`, so the ORDER matters:
  ...
  3. **Deploy to PROD and re-test** (Phase 5 prod deploy). ✅ **DEV is already done** — SPEC-FIX-06 deployed + Delete Account verified working on device 2026-07-11 (dev is ES256/JWKS; the original HS256-only code returned 401 for every real user — the bug this fix resolves).
