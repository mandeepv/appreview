...
- [ ] **Apply the `completed_sections` migration to prod** — `supabase/migrations/20260710000000_add_completed_sections.sql` via `scripts/db-push-prod.sh`; then `npm run gen:supabase-types` and commit if the generated types differ. _(Evidence: migration-list output + row count.)_
