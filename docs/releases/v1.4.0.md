# Release runbook — v1.4.0

> **⚠️ SUPERSEDED 2026-07-20 — SPEC-16 + SPEC-17 shipped in v1.3.0, not v1.4.0.**
> The launch/gate/theater (SPEC-16) and onboarding UX system (SPEC-17) work this
> runbook was written for was folded into **v1.3.0**. Its device-verification
> checklist now lives in `v1.3.0.md` (updated to the real 22-screen variant B).
> **Do NOT follow this file to ship** — it describes a checkpoint that no longer
> exists as a distinct release. Kept only as history. When the next real release
> after v1.3.0 is planned, write a fresh runbook for it. (Review findings #7/#8.)

> **INSTANCE (snapshot-in-progress).** A copy of `../RELEASE_CHECKLIST.md`, ticked AS v1.4.0 ships with a date + one line of evidence per box. Freezes after shipping — do not edit once v1.4.0 is live. Created in the SPEC-16 PR; SPEC-17 appends its device-test items to the release-specific block below. Boxes are ticked with dated evidence AT RELEASE TIME, not now.

**Release scope:** v1.4.0 is a two-spec checkpoint on the `develop` train — **SPEC-16** (launch experience: kill the green flash, silence the every-launch gate, redesign the plan-creation theater) + **SPEC-17** (onboarding redesign, appended later). The v1.4.0 version bump + `rc/1.4.0` tag happen once, after SPEC-17 merges (SPEC-16 §6).

---

## Release-specific verification for v1.4.0

### SPEC-16 — launch/gate/loading (this PR's block)

- [ ] **Cold launch on a real device (not simulator):** no green frame at any point; the native splash → JS splash handoff is imperceptible. Record 240fps slo-mo as evidence.
- [ ] **Cold launch as a confirmed subscriber:** splash → app with no plan-creation screen, no progress bar, no visible intermediate screen on a normal-latency network (the quiet gate resolves within the ~400ms reveal delay).
- [ ] **Fresh signup:** the redesigned "creating your lesson plan" theater plays exactly once, full length, then the paywall presents (single `paywall_presented` in PostHog; single `plan_theater_shown` per user).
- [ ] **Sign-out → sign-in as an existing user:** quiet gate, no theater.
- [ ] **Airplane-mode launch, unsubscribed:** quiet gate reveals the spinner after ~400ms → retry state → escape hatch (Restore / Sign out / Contact support) after ~3 attempts; all three actions work.
- [ ] **Reduce Motion enabled:** theater and splash animate as simple fades — no scaling/motion (breathing glyph still, eased progress collapses to plain steps).
- [ ] **Standard paywall regression (sandbox Apple ID):** purchase → Root; restore works; cold relaunch of a subscriber → no paywall. `grep -rn "replace('Root')" src/` spot-check noted in the PR (hits only `LoadingScreen.tsx`).

### SPEC-17 — onboarding UX system (this PR's block)

- [ ] **Walk full variant-A onboarding on device:** consistent spacing/type/cards on every screen; the progress bar starts near 0 and reaches the end at the last question.
- [ ] **Single-select screens:** tap → selected state + haptic → auto-advance; no Continue button anywhere on them; rapid double-tap advances exactly one screen.
- [ ] **Multi-select screens:** Continue is absent until a selection is made; it appears with the reveal animation; it disappears if the selection is cleared.
- [ ] **Kill the app mid-flow:** resumes at the correct screen with saved answers.
- [ ] **Variant-B scaffold screens:** render in the same system (force the `onboarding-flow` flag to `variant_b`).
- [ ] **Reduce Motion enabled:** no entrance/reveal motion; the flow is fully usable.
- [ ] **Answers land in `user_profiles` unchanged** (spot-check one signup end-to-end against dev Supabase).
...
**v1.4.0 out-of-repo note:** SPEC-16 needs NO migrations, NO edge-function changes, NO dashboard changes to function. The native splash regenerates via prebuild on the EAS build — no manual asset upload. Optional at release: add `plan_theater_shown` / `gate_wait_exceeded` to a launch-health dashboard (OPS_STATE → PostHog). After ship, confirm no new Sentry error class from the `expo-splash-screen` calls (OPS_STATE → Sentry). SPEC-17 also needs NO migrations/edge/dashboard changes to function, but has two gating/at-ship PostHog actions: **before merge**, check the `onboarding-flow` ramp (hold if a test is mid-run — DECISION 1); **at ship**, add the "SPEC-17 shipped, both arms affected" prod annotation (OPS_STATE → PostHog).
