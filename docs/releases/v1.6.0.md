# Release runbook — v1.6.0

> **INSTANCE (snapshot-in-progress).** A copy of `../RELEASE_CHECKLIST.md`, ticked AS v1.6.0 ships with a date + one line of evidence per box. Freezes after shipping — do not edit once v1.6.0 is live. Created in the SPEC-19 PR. Boxes are ticked with dated evidence AT RELEASE TIME, not now.

**Release scope:** v1.6.0 = **SPEC-19** (daily streak system — Duolingo-style retention). A day counts when the user completes ≥1 lesson section; the streak is DERIVED by a pure `computeStreak` (one automatic bridge per rolling 7 days), account-scoped and DB-backed (`daily_activity`), local-first like lesson progress. Closes the v1.2.0→v1.6.0 spec train. Ships only after v1.5.0 is live and healthy (strict version order).

> **⚠️ Submission blocker for THIS release:** the `daily_activity` migration MUST be applied to **prod** via `scripts/db-push-prod.sh` BEFORE the v1.6.0 binary goes live (Phase 4). Without it every `recordActivity` upsert returns `'failed'`, streaks silently stay local-only, and the repeated-failure threshold spams Sentry. Verify RLS afterward with the standard anon/other-user probe.

---

## Release-specific verification for v1.6.0

### SPEC-19 — daily streak system (this PR's block)

- [ ] Complete a section → "Day 1" moment; chip shows 🔥1; second section same day does not increment.
- [ ] Change device date +1 day, complete a section → 🔥2; +2 days without activity → streak shows broken per the freeze rule (one skipped day bridged, two not).
- [ ] Airplane mode: section completion still records the day locally; on reconnect the row appears in dev Supabase.
...
**v1.6.0 out-of-repo note:** SPEC-19 has a schema change (`daily_activity`). Dev migration + `npm run gen:supabase-types` must be applied before the service code is exercised (OPS_STATE → Supabase §5.1); the PR hand-added the table type to `src/types/supabase.ts` to keep it compiling — the dev regen must confirm the identical shape. **Prod push is a submission blocker (above).** No edge-function change — verify `delete-account` cleanup covers `daily_activity` (FK cascade; confirm, don't assume). Optional at release: add the streak retention card (`streak_day_recorded` / `streak_lost`) to PostHog. After ship, watch Sentry for `streak activity sync repeatedly failing`.
